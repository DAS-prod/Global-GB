...
function parseWeightKg(value?: string) {
  ...
  if (normalized.includes("gram") || /\d+(?:\.\d+)?\s*(?:g|gm)\b/.test(normalized)) {
    return number / 1000;
  }
  return number;
}
...
export async function GET() {
  // ABROAD_GOOGLE_SHEET_URL is the canonical variable. The old GLOBAL name is
  // supported temporarily so an existing Vercel environment does not break.
  const sheetUrl =
  ...
  try {
    ...
    rows.forEach((row, index) => {
      ...
      // Keep the category naming aligned with the parent Godavari Basket sheet.
      // category is preferred; parent_category is preserved separately.
      const categoryName = firstValue(row, ["category", "parent_category", "catalog", "collection"]);
      const category = slugify(categoryName);
      if (!category) return;

      const name = firstValue(row, ["name", "bundle_name", "product_name"]);
      if (!name) return;
      ...
      const priceInr = parseNumber(
        firstValue(row, ["price_inr", "bundle_price", "price", "seller_price", "1kg"])
      );
      if (priceInr <= 0) {
      ...
      const items = splitValues(firstValue(row, ["items", "products", "includes", "bundle_items"]));
      ...
      const image = firstValue(row, ["image", "image_url", "photo", "photo_url"]) || "/images/brand/logo.webp";

      bundles.push({
        id: row.id?.trim() || `${category}-${index + 1}`,
        category,
        categoryName,
    ...
    return NextResponse.json(
      {
        ...
        envLoaded: true,
        refreshedAt: new Date().toISOString(),
        error: error instanceof Error ? error.message : "Unable to load the Google Sheet catalog.",
      },
      {
        status: 502,
        headers: { "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0" },
